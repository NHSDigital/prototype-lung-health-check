...
This document summarises validation logic and error messages for `prototype_v4_3`.
...
- `phone-questionnaire`: `yes` goes to `phone-questionnaire-exit`; `no` continues to `smoking-type`.
- `smoking-type`: `none` goes to `smoking-type-exit`. If multiple tobacco types are selected, the flow asks `smoking-status-current`; if one type is selected, it asks that type-specific `smoking-status`.
- `smoking-status-current`: only shown when more than one tobacco type was selected; it records which selected types are currently smoked and then continues to `date-of-birth`.
- `smoking-status`: if the answer is `less_than_lifetime_threshold`, the flow goes to `not-eligible-for-screening`; otherwise it continues through that tobacco type sub-flow.
...
- `smoking-duration`: rendered inside the active tobacco sub-flow. It validates `age-started-smoking`, conditionally `age-stopped-smoking`, and `periods-stopped-smoking` for the active tobacco type. Current tobacco types clear `ageStoppedSmoking`; `periods-stopped-smoking` of `no` clears `yearsStoppedSmoking`.
...
- `smoking-status-current`: `smoking-status-current`
...
- `age-started-smoking`, `age-stopped-smoking`, `periods-stopped-smoking`, `smoking-status`, `smoking-frequency`, `smoking-quantity`, `smoking-change`, `smoking-frequency-change`, `smoking-quantity-change`, and `smoking-years-change` can replace their base `required` text with contextual text generated from the runtime heading.
- For headings beginning `How often`, the required message is `Select ...`; for `How old`, `How much`, `How many`, or `How long`, the message is `Enter ...` for text inputs and `Select ...` for single-choice inputs; yes/no headings become `Select whether ...`.
...
| Rule | Message | Link |
| --- | --- | --- |
| `required` | Select what level of education you have completed | `#education` |
...
Question: Have you ever smoked any of the following types of tobacco for a period of 1 year or longer?
...
Question: Do you currently smoke any of the following types of tobacco?

Type: `multiple`

Answer key: `smokingStatusCurrent`

Page: `smoking-status-current`

Validation: required

| Rule | Message | Link |
| --- | --- | --- |
| `required` | Select the types of tobacco you currently smoke | `#smoking-status-current` |
